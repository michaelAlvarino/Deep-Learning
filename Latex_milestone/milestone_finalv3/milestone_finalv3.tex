...
\begin{abstract}
   ...
   Google Research paper "Wide and Deep Learning for Recommender 	  
   Systems" to predict user ratings of movies. Unlike the Google Wide 
   and Deep model based on only categorical features, our approach 
   utilizes user sentiment analysis by incorporating text-based movie 
   reviews to enhance model prediction accuracy. 
\end{abstract}

%%%%%%%%% BODY TEXT
\section{Introduction}

	Our approach is unique in that unlike
   the Google Wide and Deep model based on only categorical features, our
   approach incorporates text-based movie reviews to enhance the our
   model's prediction accuracy. Our initial milestone recreated the 
   Google model similarly using only categorical features but with our data.
   This will serve as a baseline for model comparison and evaluation.
   Going forward, we will add in text based reviews in our model
   then experiment with convolutional and recurrent neural network architectures 
   to learn the sentiment of reviews with the goal of
   more accurately predicting movie ratings. 
   
   Source Code:: \href{https://github.com/michaelAlvarino/Deep-Learning}
   {Wide \& Deep Learning for Movie Recommendation Systems}
...
\par Recent research has focused on using Deep Learning, specifically convolutional and recurrent neural networks to help solve the generalization problem. Specifically, we will be focusing on expanding the Google Wide and Deep model thus our primary reference will be the original \href{https://arxiv.org/pdf/1606.07792.pdf}{[Cheng, Koc, et. al. 2016]} paper. Google provided a very basic tutorial outlining the general \href{https://www.tensorflow.org/tutorials/wide_and_deep}{TensorFlow} implementation of a generic Wide and Deep model. According to our knowledge no specific source code referencing the paper is open-source, however there are a few similar code-bases which we will reference \href{https://github.com/hengluchang/deep-news-summarization}{open-source examples}. Additional related papers that we referenced for constructing the Deep portion of our network include DeepFM \href{https://arxiv.org/pdf/1703.04247.pdf}{[Guo, Tang, et. al. 2017]} which similar to us, combine a matrix factorization with a CNN architecture; and DeepCoNN \href{https://arxiv.org/pdf/1701.04783.pdf}{[Zheng, Noroozi, Yu, 2017]} which uses parallel CNN architectures to predict ratings from users text reviews. 
...
From the above formulation we can clearly see the major disadvantage of this approach: sparsity. When ratings are limited the movie-to-movie matrix is mainly zeros thus limiting predictive ability. Google's \textit{Wide \& Deep Model} attempts to work around this by training two networks in parallel. Both are discussed in more detail below, but the \textbf{Wide network} is a generalized linear model of input features and transformed features. The \textbf{Deep network} is a three layer feed-forward neural network with non-linear \textit{ReLu} activation layers.  
